...
class AuthController extends ChangeNotifier {
  ...
  Future<bool> verifyUserOtp({
    ...
    try {
      ...
      state = state.copyWith(isAuthenticated: true,onboardingSeen: prefs.onboardingSeen,);

      notifyListeners();

      return true;
    } on DioException catch (e) {
      otpErrorMessage = _dioMessage(e, fallback: 'Invalid or expired OTP');
      notifyListeners();
      return false;
    } catch (_) {
 ...
 Future<void> logout() async {
  ...
  try {
    ...
    if (kDebugMode) print('Logout API failed, but clearing local data anyway');
  }

  final onboardingSeen = prefs.onboardingSeen; // ✅ save before clearing

  await tokenStorage.clear();
  await prefs.clearAll();

  await prefs.setOnboardingSeen(onboardingSeen); // ✅ restore after clear
